...
def generar_sql(dry_run=False):
    ...
    rows = conn.execute("""
        SELECT nombre, partido, camara, rating, partidas, aprobados,
               desechados, pendientes_largo, COALESCE(retiradas, 0), draws,
    ...
    for i in range(0, len(rows), BATCH):
        ...
        vals = []
        for r in batch:
            nombre, partido, camara, rating, partidas, apr, des, pend, ret, draws, fecha, leg_id, indice, pct_cam = r
            vals.append(
                f"({sql_escape(nombre)}, {sql_escape(partido)}, {sql_escape(camara)}, "
                f"{rating or 0}, {partidas or 0}, {apr or 0}, {des or 0}, {pend or 0}, "
                f"{ret or 0}, {draws or 0}, {sql_escape(fecha)}, "
        ...
        sql_parts.append(
            ...
            "(nombre, partido, camara, rating, partidas, aprobados, desechados, pendientes_largo, retiradas, draws, fecha_calculo, legislador_id, indice, percentil_camara) VALUES\n"
            + ",\n".join(vals) + ";"
        )
